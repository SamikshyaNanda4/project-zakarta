...
export const PropertyPublicSchema = z
  .object({
    ...
    description: z.string().nullable().openapi({ example: "Spacious flat near metro." }),
...
export const CreatePropertyBodySchema = z
  .object({
    ...
    contact: z.string().min(6).max(20).openapi({ example: "+91 98765 43210" }),
    price: z.string().max(50).optional().openapi({ example: "45 Lakhs" }),
    description: z.string().max(1000).optional().openapi({ example: "Spacious flat near metro." }),
  })
