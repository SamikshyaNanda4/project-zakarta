...
export function usePropertyForm() {
  ...
  // Reset sections when listing type changes
  useEffect(() => {
  ...
  // Section field arrays (derived from isSell)
  const s2Fields = section2Fields(isSell);
  ...
  async function onSubmit() {
    ...
    try {
      ...
      if (apiErr.response?.status === 403) {
        toastError("Permission denied", "Your account is not permitted to post listings.");
        return;
      }
